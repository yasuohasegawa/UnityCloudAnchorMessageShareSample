...
namespace GoogleARCore.Examples.CloudAnchor
{
    ...
    public class RoomSharingClient : NetworkClient
    {
        ...
        public void GetAnchorIdFromRoom(Int32 roomId, string ipAddress, GetAnchorIdFromRoomDelegate GetAnchorIdFromRoomCallback)
        {
            ...
            RegisterHandler(RoomSharingMsgType.AnchorIdFromRoomTextResponse, OnGetAnchorIdFromRoomTextResponse);
            RegisterHandler(RoomSharingMsgType.AnchorIdFromRoomTouchResponse, OnGetAnchorIdFromRoomTouchResponse);
        ...
        public void SendMsgToHost() {
            ...
            AnchorIdFromRoomTextRequestMessage strMessage = new AnchorIdFromRoomTextRequestMessage
            {
                msg = "From client " + temp.ToString()
            };

            Send(RoomSharingMsgType.AnchorIdFromRoomTextRequest, strMessage);
        }

        public void SendTouchToHost(int val)
        {
            float temp = UnityEngine.Random.Range(0, 100000);
            AnchorIdFromRoomTouchRequestMessage strMessage = new AnchorIdFromRoomTouchRequestMessage
            {
                touch = val
            };

            Send(RoomSharingMsgType.AnchorIdFromRoomTouchRequest, strMessage);
        }
        ...
        private void OnGetAnchorIdFromRoomTextResponse(NetworkMessage networkMessage)
        {
            var response = networkMessage.ReadMessage<AnchorIdFromRoomTextResponseMessage>();
            if(OnTextRecieve != null){
                OnTextRecieve(response.msg);
            }
        }


        private void OnGetAnchorIdFromRoomTouchResponse(NetworkMessage networkMessage)
        {
            var response = networkMessage.ReadMessage<AnchorIdFromRoomTouchResponseMessage>();
            if (OnTouchRecieve != null)
            {
                OnTouchRecieve(response.touch);
            }
        }
    }
}